...
to_plot <- filter(.data = NEI, Pollutant == "PM25-PRI" & fips == "24510")
...
# View(head(to_plot))
plot(x = to_plot$year, y = to_plot$Emissions, main = "TOTAL.EMISSION/YEAR - Baltimore.City@Maryland(fips:'24510')", xlab = "Year", ylab = "TOTAL Emissions in TON", ylim = c(min(to_plot$Emissions), max(to_plot$Emissions)))
